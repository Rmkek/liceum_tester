...
import {Col, Row, Table, Button, FormGroup, FormControl, ControlLabel} from 'react-bootstrap';
                    ...
                    <tbody className='table_color_fix'  >                        
                            {this.state.assignments.map(element =>
                                 <tr key={element.name}>
                                    <td>{element.name}</td>
                                    <td>
                                        <form ref='uploadForm' id={`uploadForm-${element.name}`} action='http://localhost:3001/api/upload-code' method='POST' encType='multipart/form-data'>
                                            <FormGroup controlId="formControlsFile">
                                                <ControlLabel>Source code</ControlLabel>
                                                <FormControl className="upload__file" type="file" name="sampleFile"/>
                                                <Button bsStyle="primary" className="button--send" id={element.name} onClick={this.onSendClick}>
                                                    Submit!
                                                </Button>
                                            </FormGroup>
                                        </form>
